...
const getMonsterTier = (level: number) => {
...
const MonsterCard: React.FC<MonsterCardProps> = ({ monster, onTrain, canTrain, trainingCosts, onLevelUp, onEnhance, showXpActions }) => {
    ...
    const tier = getMonsterTier(monster.level);
    const imagePath = `/assets/chimera_${monster.element.toLowerCase()}_${tier}.png`;
    ...
    return (
        <div className={`relative bg-slate-800 border-2 rounded-xl p-4 transition-all hover:scale-[1.02] hover:shadow-xl ${themeClass} bg-opacity-10 h-full flex flex-col`}>
            ...
            <div className="flex justify-center mb-4">
                <div className={`relative w-32 h-32 rounded-lg bg-black/20 flex items-center justify-center overflow-hidden border border-white/10`}>
                    <img
                        src={imagePath}
                        alt={`${monster.element} Chimera Tier ${tier}`}
                            ...
                            <div className="mb-3 p-2 bg-yellow-900/20 border border-yellow-500/30 rounded">
                                ...
                                {onLevelUp && (
                                    <button
                                        ...
                                        disabled={!canLevelUp}
                                        className={`w-full py-1.5 mb-1 text-xs font-bold rounded flex items-center justify-center gap-1 ${canLevelUp
                                    ...
                                    <div className="grid grid-cols-4 gap-1">
                                        {(['attack', 'defense', 'speed', 'intelligence'] as const).map(stat => (
                                            <button
                                                ...
                                                disabled={!canEnhance}
                                                className={`py-1 text-[9px] font-bold rounded ${canEnhance
